...
#'   \strong{Figure 2A}: UMAP projection of cells cultured in IL22+ medium,
#'   colored by cell-type cluster \item \strong{Figure 2B}: Dot plot depicting
...
#'   \strong{Extended data Figure 3A}: UMAP plots depicting normalized gene expression of
#'   cell-type markers in IL22+ cultured cells \item \strong{Extended data Figure 3B}: UMAP
#'   plots depicting normalized gene expression of WNT ligands in IL22+ cultured
#'   cells \item \strong{Extended data Figure 4D}: Integrated UMAP projection of all cells
#'   colored by culture condition (IL22+/IL22-) \item \strong{Extended data Figure 4E}:
#'   Alluvial plot highlighting the proportions of each enterocyte lineage cluster per
#'   culture condition (IL22+/IL22-)
#'   \item \strong{Extended data Figure 8B}: UMAP plots
...
#' @param figure Character ID of the figure to be reproduced. Accepted values are "2A",
#'   "2B", "2C", "ED_3A", "ED_3B", "ED_4D", "ED_4E" and "ED_8B" (See Details for
...
#' plot_processed(object = final.output, figure = "2A", save = T, save.dir = ".")
#' plot_processed(object = final.output, figure = "2B", save = T, save.dir = ".")
#' plot_processed(object = final.output, figure = "2C", save = T, save.dir = ".")
#' plot_processed(object = final.output, figure = "ED_3A", save = T, save.dir = ".")
#' plot_processed(object = final.output, figure = "ED_3B", save = T, save.dir = ".")
#' plot_processed(object = final.output, figure = "ED_4D", save = T, save.dir = ".")
#' plot_processed(object = final.output, figure = "ED_4E", save = T, save.dir = ".")
#' plot_processed(object = final.output, figure = "ED_8B", save = T, save.dir = ".")}
plot_processed <- function(object=NULL, figure=NA, save=FALSE, save.dir="."){

  # Check inputs
  accept.figs <- c("2A","2B","2C","ED_3A","ED_3B","ED_4D","ED_4E","ED_8B")
  ...
  if (figure == "2A"){
    ...
    # Save?
    if (save == TRUE){
      ggplot2::ggsave(plot=plot,paste0(save.dir,"/Figure_2A.pdf"), width = 8, height = 8)
    }
  ...
  if (figure == "2B"){
    ...
    if (save == TRUE){
      ggplot2::ggsave(plot=plot,paste0(save.dir,"/Figure_2B.pdf"), width = 8, height = 8)
    }
  ...
  if (figure == "ED_3A"){
    ...
    if (save == TRUE){
      ggplot2::ggsave(plot=plot,paste0(save.dir,"/Extended_Data_Fig_3A.pdf"), width = 12, height = 12)
    }
  ...
  if (figure == "ED_3B"){
    ...
    if (save == TRUE){
      ggplot2::ggsave(plot=plot,paste0(save.dir,"/Extended_Data_Fig_3B.pdf"), width = 12, height = 5)
    }
    return(plot)
  ...
  if (figure == "ED_4D"){
    ...
    if (save == TRUE){
      ggplot2::ggsave(plot=plot,paste0(save.dir,"/Extended_Data_Fig_4D.pdf"), width = 8, height = 8)
    }
    # Draw plot
    return(plot)
  ...
  if (figure == "ED_4E"){
    cell_ct <- object@meta.data %>%
      ...
      dplyr::ungroup() %>%
      dplyr::filter(Final.IDS == "EC" | Final.IDS == "Early EC")
    as.data.frame
    ...
    plot <- ggplot2::ggplot(cell_ct, ggplot2::aes(x=condition, y= percent, stratum=Final.IDS, alluvium = Final.IDS,
      ...
      ggplot2::theme(text = ggplot2::element_text(size = 28, color = "black"), axis.line = ggplot2::element_line(size = 1), panel.background = ggplot2::element_blank(),
                     axis.title.x = ggplot2::element_blank()) +
      ggplot2::ylab("Relative cell percentage (%)") +
      ggplot2::ggtitle("Percentage of enterocyte lineages") +
      ggplot2::guides(fill=ggplot2::guide_legend(title="", reverse = T, byrow = T))


    if (save == TRUE){
      ggplot2::ggsave(plot=plot, file = paste0(save.dir,"/Extended_Data_Fig_4E.pdf"), width = 7.7, height = 10)
    }
    # Draw plot
    return(plot)
  ...
  if (figure == "ED_8B"){
    ...
    if (save == TRUE){
      ggplot2::ggsave(plot=plot, file = paste0(save.dir,"/Extended_Data_Fig_8B.pdf"), width = 10, height = 10)
    }
    #Draw final plot
    return(plot)
  }
...
#'   \item \strong{Figure 3D}: Violin plots depicting normalized gene expression
#'   of HES1, CDX1, ANPEP and CYP3A4 between cells cultured in IL22+ and IL22=
#'   media (per cell-type cluster) \item \strong{Extended Data Figure 8A}:
#'   Dotplot showing the enrichment of GO biological process terms in the top
#'   IL22 upregulated genes \item \strong{Extended Data Figure 9A}: Violin plots
#'   depicting the IL22-response module score between cells cultured in IL22+
#'   and IL22- media (overall and per cluster) \item \strong{Extended Data
#'   Figure 9B}: Violin plots depicting normalized gene expression of REG1A,
#'   REG1B, DMBT1, CFI, MUC1, DEFA6 and ITLN2 between cells cultured in IL22+
#'   and IL22= media (per cell-type cluster)}
#'
#' @param object The output Seurat object generated by
#'   \code{\link{il22_response}}
#' @param dge.output Path to the DGE file generated by
#'   \code{\link{il22_response}} ("DE_IL22.txt")
#' @param figure ID of the figure to be reproduced. Accepted values are "2D","3D","ED_8A",
#'   "ED_9A and "ED_9B" (See Details for description of figure panels)
#' @param save Save the specified plot as a pdf? (default = FALSE)
#' @param save.dir Path to the directory where the output plot should be saved
#'   (default = ".")
#'
...
#'           save = T, save.dir = ".")
#' plot_il22(object = il22.output.obj, figure = "3A",save = T, save.dir = ".")
#' plot_il22(object = il22.output.obj, figure = "ED_8A", dge.output = "DE_IL22.txt",
#'           save = T, save.dir = ".")
...
plot_il22 <- function(object=NULL, dge.output=NA, figure=NA, save=FALSE, save.dir="."){
  ...
  # Check inputs
  accept.figs <- c("2D","3A","ED_8A","ED_9A","ED_9B")
  ...
  if (figure == "3A"){
    ...
    IL_topgenes <- c("HES1","CDX1", "ANPEP", "CYP3A4")
    ...
    if (save == TRUE){
      ggplot2::ggsave(plot=IL_topgenes_plot, file= paste0(save.dir,"/Figure_3A.pdf"), width = 10, height = 7.2)
    }
  ...
  # Plot Figure ED_8A
  if (figure == "ED_8A"){
    DGE_full <- read.table(dge.output)
    DGE <- DGE_full %>% dplyr::top_n(avg_log2FC, n=52) %>% dplyr::select(gene)

    kegg.res <- clusterProfiler::enrichGO(
      DGE$gene,
      OrgDb = "org.Hs.eg.db",
      keyType = "SYMBOL",
      ont = "BP",
    ...
    kegg.res2 <- clusterProfiler::simplify(kegg.res, cutoff=0.7, by="p.adjust", select_fun=min)
    DGE_plot <- enrichplot::dotplot(kegg.res2, showCategory = 10) +
      ggplot2::ggtitle("+IL-22 vs -IL-22")

    if (save == TRUE){
      ggplot2::ggsave(DGE_plot, file= paste0(save.dir,"/Extended_Data_Fig_8A.pdf"), width = 16, height = 8, units = "in")
    }
    return(DGE_plot)
  }
  ...
  if (figure == "ED_9B"){
    ...
    IL_topgenes <- c("REG1A","REG1B", "DMBT1", "CFI", "MUC1", "DEFA6", "ITLN2" )
    IL_topgenes_plot <- suppressWarnings(StackedVlnPlot(obj = object, features = IL_topgenes, split.by = "condition", cols = condition.cols, group.by = "Final.IDS") +
      ggplot2::theme(panel.grid.major = ggplot2::element_line(colour = "gray",size=0.75)))
    ...
    if (save == TRUE){
      ggplot2::ggsave(plot=IL_topgenes_plot, file= paste0(save.dir,"/Extended_Data_Fig_9B.pdf"), width = 12, height = 12)
    }
    return(suppressWarnings(IL_topgenes_plot))
  }
...
#' @param figure ID of the figure to be reproduced. Accepted values are "ED_4B"
...
plot_trajectory <- function(seurat.obj = NULL, slingshot.obj = NULL, figure = NA, save=FALSE, save.dir="."){
  ...
  # Plot Figure ED_4B (STEM trajectory, cluster + Phase)
  if (figure == "ED_4B"){
    ...
    if (save == TRUE){
      ggplot2::ggsave(plot=plot, file = paste0(save.dir,"/Extended_Data_Fig_4B.pdf"), width = 12, height = 6)
    }

    return(plot)
  }

  # Plot Extended data Figure 4C (all other trajectories)
  ...
  if (figure == "ED_4C"){
    ...
    if (save == TRUE){
      ggplot2::ggsave(plot=full.plot, file = paste0(save.dir,"/Extended_Data_Fig_4C.pdf"), width = 16, height = 5)
    }
