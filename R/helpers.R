...
#'
#' This is an identical method to that implemented in the Seurat function \code{\link[Seurat]{AddModuleScore}}, but with the ability to ensure that the control genes (per query gene) are unique
#'
#' @param expMatrix Normalized count matrix, can be generated by calling \code{\link[Seurat]{GetAssayData}}
#' @param genes.list List containing character vectors of genes to query for module score(s). A module score will be calculated for each list entry (default: NULL)
#' @param genes.pool A vector of gene symbols from which control genes can be pulled (default: NULL; all genes are fair game)
...
#' @param enrich.name Character vector specifying the name of output module score(s) (default: "Cluster")
#' @param uniqueOnly Should control gene lists be unique? (default: TRUE)
#'
